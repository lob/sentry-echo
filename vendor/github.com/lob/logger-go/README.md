...
* release
  ...
  * This is set by Nomad in staging/production environments
* timestamp
* nanoseconds

Logs are written to stdout by default. The package ships with a global logger so you can easily emit logs without having to instantiate a logger. This default logger will always write to stdout.
...
// You can also add data to the top-level of the logger (or the "root").
l1 = l1.Root(logger.Data{"rootField": "rootValue"})
l1.Info("hi")
// Outputs {"level":"info","host":"HOSTNAME","release":"RELEASE","id":"test","rootField":"rootValue","data":{"test":"data"},"nanoseconds":1531945897647586415,"timestamp":"2018-07-18T13:31:37-07:00","message":"hi"}

// If Data or Root are empty, they will not show up in the logs.
l1 = l1.ClearRoot().ClearData()
l1.Info("hi")
// Outputs {"level":"info","host":"HOSTNAME","release":"RELEASE","id":"test","nanoseconds":1531945897647586415,"timestamp":"2018-07-18T13:31:37-07:00","message":"hi"}
...
The logger supports six levels of logging.
...
There are two approaches for adding custom fields to your log output: at the root of the log and nested within a data object.  Natively, fields such as `host`, `release`, `service`, and `name` appear at the root of every log.  In most other cases, we want to inject custom metadata associated with the log in the `data` object.  However, when it comes to working with DataDog and correlating logs and traces, we need to ensure that the metadata of `dd.span_id` and `dd.trace_id` exist at the root of the log.  
...
l1 := logger.New("myservicename", logFields).ID("test")
// {"level":"error","host":"LOB-ABC","release":"","service":"test-api","name":"test-api","dd.trace_id":"8368928179613596727","dd.span_id":"8368928179613596727","nanoseconds":1733862296545269000,"status":"error","data":{"handler":"get_batch"},"timestamp":"2024-12-10T14:24:56-06:00","message":"Error "}
```
...
```go
l1 := logger.New("myservicename").ID("test")
l2 := l1.Data(logger.Data{"dd.trace_id": traceID, "dd.span_id": spanID, "test": "data"})
// {"level":"error","host":"LOB-ABC","release":"","service":"test-api","name":"test-api","nanoseconds":1733862296545269000,"status":"error","data":{"dd.trace_id":"8368928179613596727","dd.span_id":"8368928179613596727","handler":"get_batch"},"timestamp":"2024-12-10T14:24:56-06:00","message":"Error "}
```
...
To demonstrate the difference between these two mechanisms, here is an example:
...
After determining what the next version should be run the `Create Release` GitHub Action,
specifying the version update type (`patch`, `minor`, or `major`).
